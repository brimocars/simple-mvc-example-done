...
const incrementDogAge = async (req, res) => {
  ...
  try {
    const result = await Dog.findOneAndUpdate({ name }, { $inc: { age: 1 } }, { returnDocument: 'after' })
      .lean()
      .exec();
    if (result) {
      console.log(`Incremented age of dog ${name} to ${result.age}`);
      return res.status(200).json({
        result,
      });
    }
    console.log(`Dog not found: ${name}`);
    return res.status(404).json({ error: `Dog not found: ${name}` });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: `failed to increment age of dog ${name}` });
  }
};
